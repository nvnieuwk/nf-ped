package nvnieuwk.ped

import java.nio.file.Path

import groovy.transform.CompileStatic

import nvnieuwk.ped.exceptions.InvalidPedigreeException

/**
 * Implements the class for handling individual entries in a PED file.
 */
@CompileStatic
class PedEntry {

    private String family
    private String individual
    private String father
    private String mother
    private Sex sex
    private Phenotype phenotype
    private List<String> additionalFields = []

    PedEntry(List<String> parts, Integer lineNumber, Path pedFile) {
        this.family = parseID(parts[0])
        if (this.family == null) {
            throw new InvalidPedigreeException(
                "Expected a family ID at line $lineNumber in PED file '${pedFile.toUri()}'"
            )
        }
        this.individual = parseID(parts[1])
        if (this.individual == null) {
            throw new InvalidPedigreeException(
                "Expected an individual ID at line $lineNumber in PED file '${pedFile.toUri()}'"
            )
        }
        this.father = parseID(parts[2])
        this.mother = parseID(parts[3])
        this.sex = Sex.determine(parts[4])
        this.phenotype = Phenotype.determine(parts[5])
        if (parts.size() > 6) {
            this.additionalFields = parts[6..-1]
        }
    }

    String getFamily() {
        return family
    }

    void setFamily(String family) {
        this.family = parseID(family)
    }

    String getIndividual() {
        return individual
    }

    void setIndividual(String individual) {
        this.individual = parseID(individual)
    }

    String getFather() {
        return father
    }

    void setFather(String father) {
        this.father = parseID(father)
    }

    String getMother() {
        return mother
    }

    void setMother(String mother) {
        this.mother = parseID(mother)
    }

    String getSex() {
        return sex.toString()
    }

    void setSex(Integer sex) {
        setSex(sex.toString())
    }

    void setSex(String sex) {
        this.sex = Sex.determine(sex)
    }

    String getPhenotype() {
        return phenotype.toString()
    }

    void setPhenotype(Integer phenotype) {
        setPhenotype(phenotype.toString())
    }

    void setPhenotype(String phenotype) {
        this.phenotype = Phenotype.determine(phenotype)
    }

    List<String> getAdditionalFields() {
        return additionalFields
    }

    void setAdditionalFields(List<String> additionalFields) {
        this.additionalFields = additionalFields
    }

    String toString() {
        String additional = additionalFields ? '\t' + additionalFields.join('\t') : ''
        return "${family}\t${individual}\t${father ?: '0'}\t${mother ?: '0'}\t${sex}\t${phenotype}${additional}"
    }

    private final List<String> ignoreValues = ['0', 'NA', '']

    private String parseID(String id) {
        return ignoreValues.contains(id) ? null : id
    }

}

@CompileStatic
enum Sex {

    MALE,
    FEMALE,
    UNKNOWN

    static Sex determine(String value) {
        if (value == '1') {
            return MALE
        } else if (value == '2') {
            return FEMALE
        }
        return UNKNOWN
    }

    String toString() {
        switch (this) {
            case MALE: return '1'
            case FEMALE: return '2'
            case UNKNOWN: return '0'
        }
    }

}

@CompileStatic
enum Phenotype {

    AFFECTED,
    UNAFFECTED,
    MISSING

    static Phenotype determine(String value) {
        if (value == '1') {
            return UNAFFECTED
        } else if (value == '2') {
            return AFFECTED
        }
        return MISSING
    }

    String toString() {
        switch (this) {
            case UNAFFECTED: return '1'
            case AFFECTED: return '2'
            case MISSING: return '0'
        }
    }

}
